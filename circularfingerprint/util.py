...
def get_ith_minibatch_ixs(i, num_datapoints, batch_size):
    num_minibatches = int(num_datapoints / batch_size) + ((num_datapoints % batch_size) > 0)
    i = i % num_minibatches
    start = i * batch_size
...
def build_batched_grad(grad, batch_size, inputs, targets):
    ...
    def batched_grad(weights, i):
        curr_idxs = get_ith_minibatch_ixs(i, len(targets), batch_size)
        return grad(weights, inputs[curr_idxs], targets[curr_idxs])
    return batched_grad
